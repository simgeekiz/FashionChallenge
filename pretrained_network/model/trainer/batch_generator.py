...
def get_right_format(imag, session, desired_size=DESIRED_IMAGE_SIZE, color=(255, 255, 255)):
    """
    ...
    """
    img = session.run(imag)
    old_size = img.shape[0:-1]

    ratio = float(desired_size)/max(old_size)
    new_size = tuple([int(x*ratio) for x in old_size])

    im = tf.image.resize_images(img, new_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    right_format = tf.image.resize_image_with_crop_or_pad(im, desired_size, desired_size)

    return session.run(right_format)
...
class BatchSequence(Sequence):
    ...
    def __init__(self, input_dir, y, batch_size, session, desired_size=DESIRED_IMAGE_SIZE):
        """
    ...
    def __getitem__(self, idx):
        ...
        batch_x = [self.x[i] for i in idxs]
        batch_y = [self.y[i][1:] for i in idxs]  #[1:] trims off the id column
